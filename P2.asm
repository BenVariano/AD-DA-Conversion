...
;;;;;;; Assembler directives ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        ...
        __CONFIG  _CONFIG1H, _OSC_HS_1H  ;HS oscil mlator
...
Mainline
        ...
        ;LOOP_
L1
		MOVLF  B'10000111',ADCON1 ;set A/D I/O for conversion
        rcall  T40     ;wait forty microseconds
        bsf  ADCON0,1    ;set the go/done bit
		bcf ADCON2, 7  ; CLEAR
test
        btfsc   ADCON0,1
        bra test
        MOVLF  B'10001110',ADCON1  ;set A/D I/O for Display
  
        DISPLAY  ADRESH               ;Display ADRESH as a binary number
  movff ADRESH, PORTB   ;

;LOOP
 	bcf PORTC,0     ;drive low by clearing RC0
	MOVLF B'00100001',SSPBUF ; dac prefix, initiate register swap btwn master and slave
	call Slow   ;My Sub Routine to slow the program
	MOVFF ADRESH ,SSPBUF; dac data, write data from ADRESH
	call Slow
	bsf PORTC,0   ;SET BIT
;bra LOOP
        rcall  LoopTime              ;wait looptime for ten milliseconds
        ;ENDLOOP_
        bra L1
PL1
...
Initial
		MOVLF  B'00011101',ADCON0      ;Enable the A/D converter for RE2
        MOVLF  B'10001110',ADCON1      ;Enable PORTA & PORTE digital I/O pins
		MOVLF  B'00110101',ADCON2      ;set clock and acquisition time (Left Justified, 16 TAD, Fosc/16)
        MOVLF  B'11001001',TRISA       ;Set I/O for PORTA
...
T40
        movlw  133/3                   ;Each REPEAT loop takes 3 cycles
        movwf  COUNT
        ;REPEAT_
L4
          decf  COUNT,F
